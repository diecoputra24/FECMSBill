...
const CustomerStatusHistoryPage: React.FC = () => {
    const {
        requests,
        loading,
        fetchRequests,
    ...
    const handleExportExcel = async () => {
        ...
        const worksheet = workbook.addWorksheet('Perubahan Status');
        ...
        saveAs(new Blob([buffer]), `Perubahan_Status_${timestamp}.xlsx`);
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (row: CustomerStatusRequest) => (
                <div className="max-w-[150px]">
                    <p className="text-xs text-slate-500 italic line-clamp-1" title={row.reason || row.requestNote || ""}>
                        {row.reason || row.requestNote || "-"}
                    </p>
                </div>
            )
        ...
        {
            header: "PEMOHON",
            render: (row: CustomerStatusRequest) => <span className="text-sm font-medium text-slate-600">{row.requestedBy || "-"}</span>
        },
        {
            header: "STATUS",
            render: (row: CustomerStatusRequest) => (
        ...
        {
            header: "APPROVED BY",
            render: (row: CustomerStatusRequest) => <span className="text-sm font-medium text-slate-600">{row.approvedBy || "-"}</span>
        }
    ];
                ...
                {showDetail && (
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <div>
                                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mb-2">Perubahan Status</p>
                                <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-lg border border-slate-100">
                                    <div className="flex-1">
                                        <p className="text-[9px] text-slate-400 uppercase">Status Lama</p>
                                        <p className="text-sm font-medium text-slate-500 line-through">{getStatusLabel(showDetail.currentStatus)}</p>
                                    </div>
                                    <ArrowRight size={16} className="text-slate-300" />
                                    <div className="flex-1">
                                        <p className="text-[9px] text-slate-400 uppercase">Status Baru</p>
                                        <div className="flex items-center gap-1.5">
                                            {getStatusIcon(showDetail.newStatus)}
                                            <p className={`text-sm font-bold ${getStatusColor(showDetail.newStatus)}`}>{getStatusLabel(showDetail.newStatus)}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="pt-2 border-t border-slate-100">
                                <div className="flex items-center justify-between mb-1 text-left">
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Alasan / Catatan Pemohon:</span>
...
export default CustomerStatusHistoryPage;
